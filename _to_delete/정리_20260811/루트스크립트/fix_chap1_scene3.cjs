...
// I will just replace the works array for Chapter 1 with the full 5 scenes.
const worksRegex = /1: \{\s*works: \[[\s\S]*?\]\,/m;
...
// Wait, I also need to update levelOneWords, levelTwoWords, sceneSpots, levelOneSpots for Chapter 1!
// Because the current ones only have 4 arrays!
const wordsRegex = /levelOneWords: \[[\s\S]*?levelTwoWords: \[[\s\S]*?\],/m;

const newWords = `levelOneWords: [
...
content = content.replace(wordsRegex, newWords);

const spotsRegex = /sceneSpots: \[[\s\S]*?levelOneSpots: \[[\s\S]*?\]/m;
const newSpots = `sceneSpots: [
...
content = content.replace(spotsRegex, newSpots);

fs.writeFileSync(path, content, 'utf8');
console.log('Added 5 scenes back to chapter 1');
