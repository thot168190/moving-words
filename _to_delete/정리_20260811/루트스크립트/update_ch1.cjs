...
const newData = `1: {
        works: [
          ...
          {n:'06',title:'곶을 돌아 열리는 만',sub:'미지의 풍경',video:'챕터1_완성동영상/scene-ch1-10.mp4',img:'챕터1_완성동영상/scene-ch1-10-poster.jpg',words:[['adventure','모험'],['bay','만'],['cape','곶'],['channel','물길'],['coast','해안'],['flood','밀려들다'],['flow','흐르다'],['pool','웅덩이']]}
        ],
        levelOneWords: [
...
// Regex to replace the 1: { ... } block
const regex = /1:\s*\{[\s\S]*?levelOneSpots:\s*\[[\s\S]*?\]\n\s*\}/;
content = content.replace(regex, newData);

// Restore FREE_SCENES to 3
content = content.replace(/const FREE_SCENES=5;/g, 'const FREE_SCENES=3;');

fs.writeFileSync(path, content, 'utf8');
console.log('Successfully updated chapter 1 data and set FREE_SCENES=3');
