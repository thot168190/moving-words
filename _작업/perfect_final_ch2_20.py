...
PERFECT_MAP = {
    ...
    # 18. 작은 새집
    "frame": "목재 틀", "basis": "나무 밑바닥", "pole": "지지대 기둥", "hole": "작은 출입구",
    "survive": "살아가다", "settle": "보금자리 틀다", "exist": "존재하다", "rest": "쉬어가다",
    ...
    "continue": "피어나길 잇다", "regular": "규칙적 배열", "face": "해를 마주보다", "bright": "환하고 눈부심",
...
scenes = [
    ...
    ("18", "작은 새집", "새들이 쉬어가는 나무 집", ["frame", "basis", "pole", "hole"], ["survive", "settle", "exist", "rest"]),
    ...
    ("23", "키 큰 해바라기", "담장 옆 활짝 핀 꽃", ["annual", "stand", "edge", "shine"], ["continue", "regular", "face", "bright"]),
...
# all1200 단어 확인
for _, _, _, l1, l2 in scenes:
    for w in l1 + l2:
        if w not in all_1200:
            print(f"NOT IN ALL1200: {w}")
        if w not in PERFECT_MAP:
            print(f"NOT IN PERFECT_MAP: {w}")

L1_POS = [[35, 30], [72, 28], [28, 70], [68, 72]]
L2_POS = [[48, 15], [78, 52], [22, 45], [58, 85]]

all_passed = True
for n_str, title, sub, l1_w, l2_w in scenes:
    l1 = [[w, PERFECT_MAP[w], L1_POS[k]] for k, w in enumerate(l1_w)]
    l2 = [[w, PERFECT_MAP[w], L2_POS[k]] for k, w in enumerate(l2_w)]
