...
for n_str, title, sub, l1_w, l2_w in scenes_plan:
    final_l1 = []
    for k, w in enumerate(l1_w):
        if w not in all_1200 or w in curr_used:
            # find replacement
            for cand in sorted(list(free_words - curr_used)):
                if cand in DICT_1200 and DICT_1200[cand] not in curr_meanings:
                    w = cand
                    break
        curr_used.add(w)
        m = DICT_1200.get(w, f"{w}")
        curr_meanings.add(m)
        final_l1.append([w, m, L1_POS[k]])

    final_l2 = []
    for k, w in enumerate(l2_w):
        if w not in all_1200 or w in curr_used:
            for cand in sorted(list(free_words - curr_used)):
                if cand in DICT_1200 and DICT_1200[cand] not in curr_meanings:
                    w = cand
