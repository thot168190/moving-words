# -*- coding: utf-8 -*-
"""
대표님이 가져오신 최고 정본 [통나무 아래 딱정벌레 / 성인 자연사 도감 (Adult Natural-History Atlas)] 100% 이식기!

비교 분석:
1. 배경 헤더:
   - "The background is one single continuous field of pure white reaching every edge of the frame, and the subjects sit directly on that white with nothing underneath them - no sheet, no board, no panel, no card, no mat, no textured surface and no visible edge or border of any kind is ever present behind or around them."
   - "The subjects remain centered while both outer edges stay clear."
2. 0-4s 선화 문법:
   - "hair-thin pale graphite linework appears progressively from the empty white field. Throughout the whole sequence the frame contains only the flat white surface and the marks already made on it; nothing else is ever present in the frame at any moment. Each line is drawn as a moving point that travels from one end to the other, its advancing tip clearly visible the whole way, one line at a time, so the eye can follow the growing tip of every single stroke. Nothing is revealed by a sweeping wipe and nothing fades into view - every line extends from its own tip."
3. 4-7s 채색 문법:
   - "4-7s: an extremely pale, water-heavy watercolor wash develops gently. {colors}. All color remains low-saturation and transparent, with white showing through every wash. No area becomes dark, dense or fully filled. All surrounding space stays untouched pure white with no wash of any kind spreading behind the subjects."
4. 7-8s 미세 동작:
   - "7-8s: {motion}. No other subject moves, disappears, changes shape or is relocated. The finished composition still contains every subject listed above and nothing more, all lines crisp and completely stable, resting inside generous untouched white space."
5. 스타일 꼬리:
...
BEETLE_TEMPLATE = """Cinematic progressive line-reveal animation on a solid pure bright white background (#FFFFFF), edge to edge. The very first frame is an entirely empty pure white field. The background is one single continuous field of pure white reaching every edge of the frame, and the subjects sit directly on that white with nothing underneath them - no sheet, no board, no panel, no card, no mat, no textured surface and no visible edge or border of any kind is ever present behind or around them. High-key lighting. The main illustration is centered and occupies the central three-quarters of the frame, with equal narrow breathing margins on the left and right. The subjects remain centered while both outer edges stay clear. Static locked-off camera, one continuous 8-second take.

The only visible subjects throughout the sequence are {subjects}. There are no people, hands, buildings, signs or drawing tools anywhere in the scene.

0-4s: hair-thin pale graphite linework appears progressively from the empty white field. Throughout the whole sequence the frame contains only the flat white surface and the marks already made on it; nothing else is ever present in the frame at any moment. Each line is drawn as a moving point that travels from one end to the other, its advancing tip clearly visible the whole way, one line at a time, so the eye can follow the growing tip of every single stroke. Nothing is revealed by a sweeping wipe and nothing fades into view - every line extends from its own tip. {draw_steps} Most of each secondary form stays deliberately economical and free of internal lines, with strictly zero cross-hatching and zero dark shading.

4-7s: an extremely pale, water-heavy watercolor wash develops gently. {palette}. All color remains low-saturation and transparent, with white showing through every wash. No area becomes dark, dense or fully filled. All surrounding space stays untouched pure white with no wash of any kind spreading behind the subjects.

7-8s: {motion}. No other subject moves, disappears, changes shape or is relocated.

The finished composition still contains every subject listed above and nothing more, all lines crisp and completely stable, resting inside generous untouched white space.

Style: master-level fine-line illustration with exceptionally thin, precise pale graphite strokes and sophisticated control, luminous transparent watercolor, restrained tonal contrast, generous white space, sophisticated museum-quality editorial illustration for an adult natural-history atlas, mature and understated, with hairline strokes and no heavy outlines anywhere.

Audio: absolutely no audio of any kind is generated. The output is a completely silent video with an empty audio track. No music, no score, no instruments, no melody, no ambient tone, no sound effects, no foley, no narration, no voice and no background hum at any moment.

Never: text, letters, numbers, labels, arrows, diagrams, human figures, hands, pens, pencils, brushes, drawing tools, picture frames, split screens, sheets of paper, paper texture, visible paper grain, panels, mats, borders, background rectangles, repeated parallel hatching, cross-hatching, engraving-style shading, black ink masses, heavy dark outlines, grey-dominant mood, sumi-e style, dark watercolor, saturated green, pink or yellow or orange background wash, paint blooms behind subjects, fully painted surfaces, paint splatter, camera movement, cuts, fade-in or dissolve, music, background score, sound effects, ambient sound, any subject disappearing or being erased."""
...
SET04_ATLAS = [
    {
        ...
        "subjects": "one small A-frame cotton tent pitched at the optical center, one simple folding wood-and-cloth camp stool propped beside it, and three small smooth river pebbles on the clean white ground",
        ...
        "palette": "Only the palest weathered linen-cream on the tent fabric, the faintest warm pine on the stool wood, and one very light translucent stone-grey on the pebbles",
    ...
    },
    {
        ...
        "draw_steps": "The delicate cylindrical glass frame and domed brass top of the lantern are drawn first with fine open contours. The four slender crossed twigs lying on the ground follow, then the tiny inner teardrop flame.",
    ...
    },
    {
        ...
        "subjects": "one round brass pocket compass with its hinged lid open at center, and a neatly folded single sheet of topographic map resting quietly beside it",
        "draw_steps": "The circular casing and open lid of the compass are drawn with delicate hairline precision. The cardinal markings and slender needle follow, then the crisp folded edges of the paper map.",
...
set04_prompts = []
for item in SET04_ATLAS:
    prompt_text = BEETLE_TEMPLATE.format(
...
print("성인 자연사 도감(Beetle Atlas) 기반 Set 04 전면 교체 완료!")
