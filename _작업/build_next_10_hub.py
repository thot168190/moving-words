...
<head>
  ...
  <title>보는 단어장 — 2차 완전 신규 10편 벌크 허브</title>
  ...
  <div class="container">
    <div class="header-board">
      <div class="header-top">
        <div>
          <span class="badge-pass">✓ 정본 세필 수채화 문법 100% 통과</span>
          <h1 style="margin-top: 8px;">2차 완전 신규 10편 벌크 제작 허브</h1>
          <p class="sub-desc">기존 66편과 중복 0% · 미사용 단어 100% 결합 · 세필 수채화 정격</p>
        </div>
      </div>
      
      <button class="btn-bulk" onclick="copyBulk()">
        📋 2차 10편 전체 벌크 복사 (Google Flow 확장프로그램용)
      </button>
    </div>
...
for i, p in enumerate(NEXT_10_PROMPTS, 1):
    all_bulk_text.append(p["prompt"])
    p_id = p["id"]
    p_title = p["title"]
    p_chapter = p["chapter"]
...
html_content += f"""
  ...
  <script>
    ...
    function copyBulk() {{
      const text = bulkData.replace(/\\\\n/g, '\\n');
      navigator.clipboard.writeText(text).then(() => {{
        showToast('2차 10편 벌크 프롬프트가 복사되었습니다!');
      }});
    }}
